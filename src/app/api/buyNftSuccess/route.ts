...
export async function POST (req: NextRequest) {
  try {
    ...
    const url = `https://api.sentx.io/v1/affiliate/market/buynftres?apikey=${affiliateKeySentx}`
    ...
    console.log('Purchase confirmation result:', confirmationResult)
    return NextResponse.json(confirmationResult)
  } catch (error) {
    console.error('Error confirming purchase to market provider:', error)
