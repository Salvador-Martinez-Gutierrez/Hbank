'use client'
import { useState, useEffect } from 'react'
import type { KabilaActivity } from '@/app/services/getActivityKabila'
import getActivityKabila from '@/app/services/getActivityKabila'
import { Button } from '@/app/collections/components/ui/button'
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow
} from '@/app/collections/components/ui/table'
import { Badge } from '@/app/collections/components/ui/badge'
import Link from 'next/link'

interface KabilaActivityProps {
  tokenId: string
}

const ActivityKabila: React.FC<KabilaActivityProps> = ({ tokenId }) => {
  const [activity, setActivity] = useState<KabilaActivity[]>([])
  const [isLoading, setIsLoading] = useState(false)
  const [skip, setSkip] = useState(0)

  useEffect(() => {
    const fetchActivity = async () => {
      setIsLoading(true)
      try {
        const data = await getActivityKabila(tokenId, skip, 25)
        if (data !== null) {
          setActivity((prevActivity) => {
            const newActivity = data.filter(newItem =>
              !prevActivity.some(existingItem =>
                existingItem.serialNumber === newItem.serialNumber &&
                existingItem.createdAt === newItem.createdAt
              )
            )
            return [...prevActivity, ...newActivity]
          })
        }
      } catch (error) {
        console.error('Error fetching activity:', error)
      } finally {
        setIsLoading(false)
      }
    }

    fetchActivity()
  }, [tokenId, skip])

  if (!isLoading && activity.length === 0) {
    return (
      <div className="text-grey-500 flex items-center gap-2 border-2 border-zinc-800 rounded-md p-3 mt-6">
        <p>No activity found for this NFT Collection in the Kabila Market</p>
      </div>
    )
  }

  return (
    <main className="w-full">
      <div className="overflow-x-auto">
        <Table>
          <TableHeader>
            <TableRow className='hover:bg-neutral-900 bg-neutral-900 sticky z-10'>
              <TableHead>NFT</TableHead>
              <TableHead>Type</TableHead>
              <TableHead>Price</TableHead>
              <TableHead>Seller</TableHead>
              <TableHead>Buyer</TableHead>
              <TableHead>Date</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {activity?.map((item, index) => (
              <TableRow key={index} className="hover:bg-neutral-800">
                <TableCell className="whitespace-nowrap">
                  <div className='flex flex-col overflow-hidden'>
                    <span className="font-medium">{item.name}</span>
                    <span className="text-muted-foreground text-sm">#{item.serialNumber}</span>
                  </div>
                </TableCell>
                <TableCell>
                  <Badge
                    variant='secondary'
                    className={`uppercase ${
                      item.activityType.toLowerCase() === 'sale'
                        ? 'border-green-500 text-green-500 bg-transparent'
                        : 'border-white text-white bg-transparent'
                    }`}
                  >
                    {item.activityType.toLowerCase() === 'sale'
                      ? 'Sold'
                      : item.activityType.toLowerCase() === 'listing'
                        ? 'Listed'
                        : item.activityType}
                  </Badge>
                </TableCell>
                <TableCell>{`${item.price}ℏ`}</TableCell>
                <TableCell className="whitespace-nowrap truncate text-left">
                  <Link
                    href={`/address/${item.sellerId}`}
                    className="hover:text-blue-500 transition-colors"
                  >
                    {item.sellerId}
                  </Link>
                </TableCell>
                <TableCell className="whitespace-nowrap truncate text-left">
                  <Link
                    href={`/address/${item.buyerId}`}
                    className="hover:text-blue-500 transition-colors"
                  >
                    {item.buyerId}
                  </Link>
                </TableCell>
                <TableCell>{new Date(item.createdAt).toLocaleDateString()}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </div>

      <div className="flex flex-col items-center mt-4">
        {isLoading && (
          <div className="mb-4">
            <strong>Loading...</strong>
          </div>
        )}

        {!isLoading && (
          <Button
            onClick={() => { setSkip(skip + 25) }}
            className='bg-neutral-900 hover:bg-neutral-800 inline-flex cursor-pointer justify-start my-4 px-16 py-2 text-gray-300 text-sm border border-gray-300'
          >
            Load More
          </Button>
        )}
      </div>
    </main>
  )
}

export default ActivityKabila
